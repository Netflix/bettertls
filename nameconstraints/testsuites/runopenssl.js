...
runner.runSystem('openssl version', function(version) {
  ...
  runner.runTests(version, function(dnsUrl, ipUrl, done) {
    ...
    runner.runSystem('openssl s_client -CAfile ../../docs/nameconstraints/root.crt -verify_hostname ' + matches[1] + ' -connect ' + matches[1] + ':' + matches[2] + ' </dev/null', function(dnsOutput) {
      ...
      runner.runSystem('openssl s_client -CAfile ../../docs/nameconstraints/root.crt -verify_ip ' + matches[1] + ' -connect ' + matches[1] + ':' + matches[2] + ' </dev/null', function(ipOutput) {
  ...
  }, '../../docs/nameconstraints/results/openssl_1.1.0f_linux.json');
});
